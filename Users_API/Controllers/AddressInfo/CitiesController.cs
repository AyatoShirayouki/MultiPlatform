...
namespace Users_API.Controllers.AddressInfo
{
    ...
    public class CitiesController : ControllerBase
    {
        ...
        public CitiesController(IOptionsMonitor<JwtConfig> optionsMonitor, TokenValidationParameters tokenValidationParams, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IActionResult> GetAll([FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (token != null && refreshToken != null)
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await CitiesManagementService.GetAll();
        ...
        [HttpPost]
        [Route("Save")]
        public async Task<IActionResult> Save(CityDTO cityDTO, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (cityDTO == null || token == null || refreshToken == null)
            {
                response.Code = 400;
                response.Error = "Missing data - City and/or token and/or refresh token is null or invalid!";
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await CitiesManagementService.Save(cityDTO);
                    response.Body = "City has been saved!";
        ...
        public async Task<IActionResult> GetById(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await CitiesManagementService.GetById(id);
        ...
        public async Task<IActionResult> Delete(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtAdminToken.JwtSuccess == true)
                {
                    ...
                    await CitiesManagementService.Delete(id);
                    response.Body = "City has been deleted!";
