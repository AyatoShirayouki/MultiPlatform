...
namespace Users_API.Controllers.Education.EducationDetails
{
    ...
    public class AcademicFieldsController : ControllerBase
    {
        ...
        public AcademicFieldsController(IOptionsMonitor<JwtConfig> optionsMonitor, TokenValidationParameters tokenValidationParams, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IActionResult> GetAll([FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (token != null && refreshToken != null)
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await AcademicFieldsManagementService.GetAll();
        ...
        [HttpPost]
        [Route("Save")]
        public async Task<IActionResult> Save(AcademicFieldDTO academicFieldDTO, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (academicFieldDTO == null || token == null || refreshToken == null)
            {
                response.Code = 400;
                response.Error = "Missing data - academic field and/or token and/or refresh token is null or invalid!";
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await AcademicFieldsManagementService.Save(academicFieldDTO);
                    response.Body = "academic field has been saved!";
        ...
        public async Task<IActionResult> GetById(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await AcademicFieldsManagementService.GetById(id);
        ...
        public async Task<IActionResult> Delete(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtAdminToken.JwtSuccess == true)
                {
                    ...
                    await AcademicFieldsManagementService.Delete(id);
                    response.Body = "academic field has been deleted!";
