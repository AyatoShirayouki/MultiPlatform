...
namespace Users_API.Controllers.Education.EducationDetails
{
    ...
    [ApiController]
    public class EducationalFacilitiessController : ControllerBase
    {
        ...
        public EducationalFacilitiessController(IOptionsMonitor<JwtConfig> optionsMonitor, TokenValidationParameters tokenValidationParams, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IActionResult> GetAll([FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (token != null && refreshToken != null)
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await EducationalFacilityTypesManagementService.GetAll();
        ...
        [HttpPost]
        [Route("Save")]
        public async Task<IActionResult> Save(EducationalFacilityDTO educationalFacilityDTO, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (educationalFacilityDTO == null || token == null || refreshToken == null)
            {
                response.Code = 400;
                response.Error = "Missing data - educational facility and/or token and/or refresh token is null or invalid!";
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await EducationalFacilityTypesManagementService.Save(educationalFacilityDTO);
                    response.Body = "educational facility has been saved!";
        ...
        public async Task<IActionResult> GetById(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await EducationalFacilityTypesManagementService.GetById(id);
        ...
        public async Task<IActionResult> Delete(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtAdminToken.JwtSuccess == true)
                {
                    ...
                    await EducationalFacilityTypesManagementService.Delete(id);
                    response.Body = "educational facility has been deleted!";
