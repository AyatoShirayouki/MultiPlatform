...
namespace Freelance_API.Controllers.TaskRelated
{
    ...
    [ApiController]
    public class FilesToTasksController : ControllerBase
    {
        ...
        public FilesToTasksController(IOptionsMonitor<JwtConfig> optionsMonitor, TokenValidationParameters tokenValidationParams, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IActionResult> GetAll([FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (token != null && refreshToken != null)
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await FilesToTasksManagementService.GetAll();
        ...
        [HttpPost]
        [Route("Save")]
        public async Task<IActionResult> Save(FileToTaskDTO fileToTaskDTO, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (fileToTaskDTO == null || token == null || refreshToken == null)
            {
                response.Code = 400;
                response.Error = "Missing data - fileToTask and/or token and/or refresh token is null or invalid!";
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await FilesToTasksManagementService.Save(fileToTaskDTO);
                    response.Body = "fileToTask has been saved!";
        ...
        public async Task<IActionResult> GetById(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await FilesToTasksManagementService.GetById(id);
        ...
        public async Task<IActionResult> Delete(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await FilesToTasksManagementService.Delete(id);
                    response.Body = "fileToTask has been deleted!";
