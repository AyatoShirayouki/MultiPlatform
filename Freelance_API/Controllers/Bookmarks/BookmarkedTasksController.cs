...
namespace Freelance_API.Controllers.Bookmarks
{
    ...
    [ApiController]
    public class BookmarkedTasksController : ControllerBase
    {
        ...
        public BookmarkedTasksController(IOptionsMonitor<JwtConfig> optionsMonitor, TokenValidationParameters tokenValidationParams, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IActionResult> GetAll([FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (token != null && refreshToken != null)
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await BookmarkedTasksManagementService.GetAll();
        ...
        [HttpPost]
        [Route("Save")]
        public async Task<IActionResult> Save(BookmarkedTaskDTO bookmarkedTaskDTO, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (bookmarkedTaskDTO == null || token == null || refreshToken == null)
            {
                response.Code = 400;
                response.Error = "Missing data - bookmarkedTask and/or token and/or refresh token is null or invalid!";
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await BookmarkedTasksManagementService.Save(bookmarkedTaskDTO);
                    response.Body = "bookmarkedTask has been saved!";
        ...
        public async Task<IActionResult> GetById(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await BookmarkedTasksManagementService.GetById(id);
        ...
        public async Task<IActionResult> Delete(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await BookmarkedTasksManagementService.Delete(id);
                    response.Body = "bookmarkedTask has been deleted!";
