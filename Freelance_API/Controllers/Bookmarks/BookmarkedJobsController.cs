...
namespace Freelance_API.Controllers.Bookmarks
{
    ...
    [ApiController]
    public class BookmarkedJobsController : ControllerBase
    {
        ...
        public BookmarkedJobsController(IOptionsMonitor<JwtConfig> optionsMonitor, TokenValidationParameters tokenValidationParams, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IActionResult> GetAll([FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (token != null && refreshToken != null)
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await BookmarkedJobsManagementService.GetAll();
        ...
        [HttpPost]
        [Route("Save")]
        public async Task<IActionResult> Save(BookmarkedJobDTO bookmarkedJobDTO, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (bookmarkedJobDTO == null || token == null || refreshToken == null)
            {
                response.Code = 400;
                response.Error = "Missing data - bookmarkedJob and/or token and/or refresh token is null or invalid!";
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await BookmarkedJobsManagementService.Save(bookmarkedJobDTO);
                    response.Body = "bookmarkedJob has been saved!";
        ...
        public async Task<IActionResult> GetById(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await BookmarkedJobsManagementService.GetById(id);
        ...
        public async Task<IActionResult> Delete(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await BookmarkedJobsManagementService.Delete(id);
                    response.Body = "bookmarkedJob has been deleted!";
