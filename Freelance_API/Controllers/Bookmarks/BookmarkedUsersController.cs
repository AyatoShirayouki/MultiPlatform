...
namespace Freelance_API.Controllers.Bookmarks
{
    [Route("api/[controller]")]
    [Produces("application/json")]
    [ApiController]
    public class BookmarkedUsersController : ControllerBase
    {
        ...
        public BookmarkedUsersController(IOptionsMonitor<JwtConfig> optionsMonitor, TokenValidationParameters tokenValidationParams, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IActionResult> GetAll([FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (token != null && refreshToken != null)
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await BookmarkedUsersManagementService.GetAll();
        ...
        [HttpPost]
        [Route("Save")]
        public async Task<IActionResult> Save(BookmarkedUserDTO bookmarkedUserDTO, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (bookmarkedUserDTO == null || token == null || refreshToken == null)
            {
                response.Code = 400;
                response.Error = "Missing data - bookmarkedUser and/or token and/or refresh token is null or invalid!";
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await BookmarkedUsersManagementService.Save(bookmarkedUserDTO);
                    response.Body = "bookmarkedUser has been saved!";
        ...
        public async Task<IActionResult> GetById(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await BookmarkedUsersManagementService.GetById(id);
        ...
        public async Task<IActionResult> Delete(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await BookmarkedUsersManagementService.Delete(id);
                    response.Body = "bookmarkedUser has been deleted!";
