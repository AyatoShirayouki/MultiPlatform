...
namespace Freelance_API.Controllers.Others
{
    ...
    public class TagsController : ControllerBase
    {
        ...
        public TagsController(IOptionsMonitor<JwtConfig> optionsMonitor, TokenValidationParameters tokenValidationParams, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IActionResult> GetAll([FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (token != null && refreshToken != null)
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await TagsManagementService.GetAll();
        ...
        public async Task<IActionResult> Save(TagDTO tagDTO, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            {
                response.Code = 400;
                response.Error = "Missing data - tag and/or token and/or refresh token is null or invalid!";
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await TagsManagementService.Save(tagDTO);
                    response.Body = "tag has been saved!";
        ...
        public async Task<IActionResult> GetById(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await TagsManagementService.GetById(id);
        ...
        public async Task<IActionResult> Delete(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await TagsManagementService.Delete(id);
                    response.Body = "tag has been deleted!";
