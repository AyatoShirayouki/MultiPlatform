...
namespace Freelance_API.Controllers.Others
{
    ...
    public class SkillsController : ControllerBase
    {
        ...
        public SkillsController(IOptionsMonitor<JwtConfig> optionsMonitor, TokenValidationParameters tokenValidationParams, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IActionResult> GetAll([FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (token != null && refreshToken != null)
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await SkillsManagementService.GetAll();
        ...
        [HttpPost]
        [Route("Save")]
        public async Task<IActionResult> Save(SkillDTO skillDTO, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (skillDTO == null || token == null || refreshToken == null)
            {
                response.Code = 400;
                response.Error = "Missing data - skill and/or token and/or refresh token is null or invalid!";
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await SkillsManagementService.Save(skillDTO);
                    response.Body = "skill has been saved!";
        ...
        public async Task<IActionResult> GetById(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await SkillsManagementService.GetById(id);
        ...
        public async Task<IActionResult> Delete(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await SkillsManagementService.Delete(id);
                    response.Body = "skill has been deleted!";
