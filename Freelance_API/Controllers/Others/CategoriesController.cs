...
namespace Freelance_API.Controllers.Others
{
    ...
    public class CategoriesController : ControllerBase
    {
        ...
        public CategoriesController(IOptionsMonitor<JwtConfig> optionsMonitor, TokenValidationParameters tokenValidationParams, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IActionResult> GetAll([FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (token != null && refreshToken != null)
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await CategoriesManagementService.GetAll();
        ...
        [HttpPost]
        [Route("Save")]
        public async Task<IActionResult> Save(CategoryDTO categoryDTO, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (categoryDTO == null || token == null || refreshToken == null)
            {
                response.Code = 400;
                response.Error = "Missing data - category and/or token and/or refresh token is null or invalid!";
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await CategoriesManagementService.Save(categoryDTO);
                    response.Body = "category has been saved!";
        ...
        public async Task<IActionResult> GetById(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await CategoriesManagementService.GetById(id);
        ...
        public async Task<IActionResult> Delete(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await CategoriesManagementService.Delete(id);
