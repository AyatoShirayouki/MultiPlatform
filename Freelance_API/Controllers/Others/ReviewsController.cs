...
namespace Freelance_API.Controllers.Others
{
    ...
    public class ReviewsController : ControllerBase
    {
        ...
        public ReviewsController(IOptionsMonitor<JwtConfig> optionsMonitor, TokenValidationParameters tokenValidationParams, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IActionResult> GetAll([FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (token != null && refreshToken != null)
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await ReviewsManagementService.GetAll();
        ...
        [HttpPost]
        [Route("Save")]
        public async Task<IActionResult> Save(ReviewDTO reviewDTO, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (reviewDTO == null || token == null || refreshToken == null)
            {
                response.Code = 400;
                response.Error = "Missing data - review and/or token and/or refresh token is null or invalid!";
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await ReviewsManagementService.Save(reviewDTO);
                    response.Body = "review has been saved!";
        ...
        public async Task<IActionResult> GetById(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await ReviewsManagementService.GetById(id);
        ...
        public async Task<IActionResult> Delete(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await ReviewsManagementService.Delete(id);
                    response.Body = "review has been deleted!";
