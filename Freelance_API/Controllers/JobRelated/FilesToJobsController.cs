...
namespace Freelance_API.Controllers.JobRelated
{
    ...
    [ApiController]
    public class FilesToJobsController : ControllerBase
    {
        ...
        public FilesToJobsController(IOptionsMonitor<JwtConfig> optionsMonitor, TokenValidationParameters tokenValidationParams, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IActionResult> GetAll([FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (token != null && refreshToken != null)
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await FilesToJobsManagementService.GetAll();
        ...
        [HttpPost]
        [Route("Save")]
        public async Task<IActionResult> Save(FileToJobDTO fileToJobDTO, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (fileToJobDTO == null || token == null || refreshToken == null)
            {
                response.Code = 400;
                response.Error = "Missing data - fileToJob and/or token and/or refresh token is null or invalid!";
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await FilesToJobsManagementService.Save(fileToJobDTO);
                    response.Body = "fileToJob has been saved!";
        ...
        public async Task<IActionResult> GetById(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await FilesToJobsManagementService.GetById(id);
        ...
        public async Task<IActionResult> Delete(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await FilesToJobsManagementService.Delete(id);
                    response.Body = "fileToJob has been deleted!";
