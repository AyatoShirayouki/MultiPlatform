...
namespace Freelance_API.Controllers.JobRelated
{
    ...
    public class JobApplicationsController : ControllerBase
    {
        ...
        public JobApplicationsController(IOptionsMonitor<JwtConfig> optionsMonitor, TokenValidationParameters tokenValidationParams, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IActionResult> GetAll([FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (token != null && refreshToken != null)
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await JobApplicationsManagementService.GetAll();
        ...
        [HttpPost]
        [Route("Save")]
        public async Task<IActionResult> Save(JobApplicationDTO jobApplicationDTO, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (jobApplicationDTO == null || token == null || refreshToken == null)
            {
                response.Code = 400;
                response.Error = "Missing data - jobApplication and/or token and/or refresh token is null or invalid!";
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await JobApplicationsManagementService.Save(jobApplicationDTO);
                    response.Body = "jobApplication has been saved!";
        ...
        public async Task<IActionResult> GetById(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await JobApplicationsManagementService.GetById(id);
        ...
        public async Task<IActionResult> Delete(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await JobApplicationsManagementService.Delete(id);
                    response.Body = "jobApplication has been deleted!";
