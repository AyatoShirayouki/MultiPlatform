...
namespace Freelance_API.Controllers.JobRelated
{
    ...
    public class JobTypesController : ControllerBase
    {
        ...
        public JobTypesController(IOptionsMonitor<JwtConfig> optionsMonitor, TokenValidationParameters tokenValidationParams, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IActionResult> GetAll([FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (token != null && refreshToken != null)
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await JobTypesManagementService.GetAll();
        ...
        [HttpPost]
        [Route("Save")]
        public async Task<IActionResult> Save(JobTypeDTO jobTypeDTO, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (jobTypeDTO == null || token == null || refreshToken == null)
            {
                response.Code = 400;
                response.Error = "Missing data - jobType and/or token and/or refresh token is null or invalid!";
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await JobTypesManagementService.Save(jobTypeDTO);
                    response.Body = "jobType has been saved!";
        ...
        public async Task<IActionResult> GetById(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await JobTypesManagementService.GetById(id);
        ...
        public async Task<IActionResult> Delete(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await JobTypesManagementService.Delete(id);
                    response.Body = "jobType has been deleted!";
